...
Фильтр "Цена за сутки" установлен на максимум 50 000 ₽, но в результатах поиска отображаются например объявления с ценой 80 000 ₽ и 130 000 ₽.
...
## 16. Фильтр посуточно, но выводит помесячно
Одно объявление с ценой за месяц, это означает, что поиск работает некорректно.

Приоритет: **Medium**

![](taskOnePage/bug16.png)

## 17. Некорректное объявление недвижимости, с пометкой о возрате
Видно маркер «14 дней на возрат», указывающее на возрат, но объявление о недвижимости. 
Пользователь не понимает, и может запутся.

Приоритет: **Low**

![](taskOnePage/bug17.png)

## 18. Не везде указана цена за весь период.
Не у всех объектов есть цена за весь период (например, у "Таунхаус 150 м²" отсутствует).
(Некорректный расчёт цены за весь период (например, "5-к. дом, 300 м²" показывает  за весь период, но если цена за сутки 21 667 ₽, то за 3 дня должно быть 21 667 ₽ × 3 = 65 001 ₽, если стоит округление до целого, тогда претензий нет)

Приоритет: **Low** 

![](taskOnePage/bug18.png)
